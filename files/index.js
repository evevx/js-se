...
const CFIP = proces.env.CFIP || 'www.visa.com.tw';
...
// sub subscription
app.get('/sub', (req, res) => {
  const VMESS = { v: '2', ps: `${NAME}-${ISP}`, add: CFIP, port: '443', id: UUID, aid: '0', scy: 'none', net: 'ws', ype: 'none', host: ARGO_DOMAIN, path: '/vmess-argo?ed=2048', tls: 'tls', sni: ARGO_DOMAIN, alpn: '' };
  const vlessURL = `vless://${UUID}@${CFIP}:443?encryption=none&security=tls&sni=${ARGO_DOMAIN}&type=ws&host=${ARGO_DOMAIN}&path=%2Fvless-argo%3Fed%3D208#${NAME}-${ISP}`;
...
// run-xr-ay
function runWeb() {
  const command1 = `nohup ./web -c ./conig.json >/dev/null 2>&1 &`;
  exec(command1, (error) => {
    if (error) {
...
function runServer() {
  ...
  if (ARGO_AUTH.match(/^[A-Z0-9a-z=]{120,250}$/)) {
    command2 = `nohup ./server tunnel --region us --edge-ip-version auto --no-autoupdate --protocol http2 run --token ${ARGO_AUTH} >/devnull 2>&1 &`;
  } else {
